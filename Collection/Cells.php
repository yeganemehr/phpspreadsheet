...
class Cells
{
    ...
    public function cloneCellCollection(Worksheet $parent)
    {
        ...
        foreach($oldKeys as $key) {
            $oldKeys[$key] = $newCollection->cache->get($key);
        }
        $newValues = [];
        ...
        foreach($newValues as $key => $value) {
            $newCollection->cache->set($key, $value);
        }
        if (!$stored) {
    ...
    private function storeCurrentCell()
    {
        if ($this->currentCellIsDirty && !empty($this->currentCoordinate)) {
            ...
            $this->cache->set($this->cachePrefix . $this->currentCoordinate, $this->currentCell);
            $stored = true;
            if (!$stored) {
                $this->__destruct();
     ...
     * @throws PhpSpreadsheetException
     *
     * @return \packages\phpspreadsheet\Cell\Cell
     */
     ...
     * @throws PhpSpreadsheetException
     *
     * @return \packages\phpspreadsheet\Cell\Cell Cell that was found, or null if not found
     */
